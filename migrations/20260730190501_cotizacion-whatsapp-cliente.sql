-- R2: the WhatsApp agent recognizes registered customers.
--
-- crear_cotizacion_whatsapp v2: resolve the incoming phone against
-- customer_phones_all (primary + extras, normalized), stamp
-- cotizaciones.customer_id, and apply the customer's descuento_pct to each
-- line. Matching is by the LAST 10 digits: WhatsApp sends 521477…/52477…
-- while the registry stores 10-digit local numbers.
--
-- Discount rounding convention (first application of descuento_pct anywhere):
...
BEGIN
  ...
  LOOP
    ...
    -- Registered-customer price: their standing discount, applied per unit.
    v_unit := CASE
