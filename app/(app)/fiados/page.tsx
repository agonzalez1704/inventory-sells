...
export default async function FiadosPage({
  ...
  let fiadosQuery = insforge.database
    .from("sales")
    .select(
      "id, total_cents, note, created_at, sold_by, fiado_publico, customer_id, customers(id, nombre, telefono, is_system, credito_dias), sale_items(product_id, qty, products(name, sku)), sale_pagos(monto_cents)",
    )
    .eq("status", "pending")
