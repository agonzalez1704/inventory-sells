-- fusionar_clientes v3: refuse a merge that would cross another one.
--
-- Two merges of the same pair ran within the same second, in opposite
-- directions (one from the screen, one from a script). Each archived the
-- other's survivor, and the pair ended up pointing at each other with nobody
-- active. The moves themselves were safe — the history landed on one record —
-- but the result needed fixing by hand.
--
-- So: a merge now refuses an archived or already-merged record on either side.
-- The second merge of a pair finds its partner archived and stops.
...
BEGIN
  ...
  END IF;
  IF v_c.fusionado_en IS NOT NULL OR NOT v_c.is_active THEN
    RAISE EXCEPTION 'el cliente que se queda (%) está archivado o ya fue combinado', v_c.nombre
      USING errcode = '23514';
  END IF;
  IF v_a.fusionado_en IS NOT NULL OR NOT v_a.is_active THEN
    RAISE EXCEPTION '% ya está archivado o combinado: recarga la lista', v_a.nombre
      USING errcode = '23514';
  END IF;
