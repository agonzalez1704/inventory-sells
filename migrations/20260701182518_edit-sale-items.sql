-- Change the product(s) on an already-registered sale (a customer swaps to a
-- different model). Same atomic swap as editar_fiado, but for any non-void sale
-- (completed or pending): restore the current items to stock, take the new set,
-- recompute the total — rejecting oversell. Payment method / status are kept.
CREATE OR REPLACE FUNCTION public.editar_items(
  p_sale_id uuid,
  p_items   jsonb
)
RETURNS uuid
...
REVOKE EXECUTE ON FUNCTION public.editar_items(uuid, jsonb) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.editar_items(uuid, jsonb) TO authenticated;
