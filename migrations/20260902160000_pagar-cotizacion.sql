-- The WhatsApp sale, end to end without a human relay. The agent quotes; the
-- customer opens their quote link, and instead of "authorize and wait for a
-- seller to call", the SAME page turns the quote into a storefront order:
-- transfer payment, pickup (send your own Uber) or shipping. The quoted prices
-- — customer discount included — are the order's prices, verbatim.

ALTER TABLE public.ordenes_web
  ADD COLUMN IF NOT EXISTS cotizacion_id uuid REFERENCES public.cotizaciones(id);

-- Proofs get a third owner: the web order awaiting its transfer confirmation.
ALTER TABLE public.comprobantes_pago
  ADD COLUMN IF NOT EXISTS orden_id uuid REFERENCES public.ordenes_web(id) ON DELETE CASCADE;
...
ALTER TABLE public.comprobantes_pago ADD CONSTRAINT comprobantes_pago_un_dueno
  CHECK (num_nonnulls(sale_id, adelanto_id, orden_id) = 1);
CREATE INDEX IF NOT EXISTS comprobantes_pago_orden_idx
  ON public.comprobantes_pago (orden_id) WHERE orden_id IS NOT NULL;

-- ---------------------------------------------------------------------------
-- Quote → web order, atomically. Reserves stock exactly like crear_orden_web
-- (dropship lines exempt), but prices come from the QUOTE's lines — that is
-- the deal the agent made, discount and all. Idempotent per quote: a second
-- click returns the order that already exists instead of reserving twice.
...
-- ---------------------------------------------------------------------------
-- When the order pays, its quote is CONVERTED — the whole chain closes itself.
...
BEGIN
  ...
  IF v_o.cotizacion_id IS NOT NULL THEN
    UPDATE public.cotizaciones
       SET estado = 'convertida', sale_id = v_sale_id
     WHERE id = v_o.cotizacion_id AND estado <> 'convertida';
  END IF;
